...
grafico3 <- ggplot(df_terrenos_frequentes, aes(x = setting_terrain)) + 
  ...
  labs(title = "Gráfico de barras da ativação de primeira da Armadilha de acordo com o Terreno", x = "Terreno", y = "Frequência relativa", fill = "Ativação de primeira da Armadilha") + 
  theme_estat()
...
```{r}
grafico_plus <- ggplot(df_terrenos_frequentes, aes(x = setting_terrain, fill = trap_work_first)) +
  geom_bar(position = "stack") +
  labs(title = "Ativação da Armadilha pela Primeira Vez por Tipo de Terreno",
       x = "Tipo de Terreno",
       y = "Contagem") +
  scale_fill_manual(values = c("red", "green"), labels = c("Não Funcionou", "Funcionou"))
grafico_plus
```
